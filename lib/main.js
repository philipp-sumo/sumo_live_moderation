    ...
    var questionRequest = Request({
        ...
        onComplete: function(response) {
			...
			if (ss.storage.lastpage == latestsumoquestionid) {
				...
				console.log("Last question dealt with was " + ss.storage.lastpage + ". Couldn't fetch update now, try again later...");
			}
				...
				if (ss.storage.lastpage < latestsumoquestionid && latestsumoquestionid - ss.storage.lastpage < 13) {
					for (var i = 0; i < response.json.results.length; i++) {
						if (response.json.results[i].id == ss.storage.lastpage) {
							sumoquestionid = response.json.results[i - 1].id;
							sumoquestioncreator = response.json.results[i - 1].creator;
							sumoquestiontitle = response.json.results[i - 1].title;
						}
					}
				}
				/*start fresh if no prior threads were looked at or they are too out of date*/
					...
					sumoquestioncreator = response.json.results[0].creator;
...
function classifyQuestion(response, sumoquestiontitle, sumoquestioncreator, sumoquestioncontent) {
    ...
    else if (prefs.alertduplicate === true && getQuestionCount(response, sumoquestioncreator) > 1) {
...
function fetchReply() {
    if (prefs.checkreplies === true){
		var replyRequest = Request({
			url: "https://support.mozilla.org/api/2/answer/?format=json",
			onComplete: function(rresponse) {
				...
				if (ss.storage.lastreply == latestsumoreplyid) {
					...
					console.log("Last reply dealt with was " + ss.storage.lastreply + ". Couldn't fetch update now, try again later...");
				}
					...
					if (ss.storage.lastreply < latestsumoreplyid && latestsumoreplyid - ss.storage.lastreply < 20) {
						for (var i = 0; i < rresponse.json.results.length; i++) {
							/*find the oldest reply not looked at yet and increase id by one*/
							if (rresponse.json.results[i].id == ss.storage.lastreply) {
								sumoreplyid = rresponse.json.results[i - 1].id;
								sumoreplycreator = rresponse.json.results[i - 1].creator;
								sumoreplyinquestion = rresponse.json.results[i - 1].question;
							}
						}
					}
					else {
						sumoreplyid = rresponse.json.results[0].id;
						sumoreplycreator = rresponse.json.results[0].creator;
...
function classifyReply(sumoreplycontent) {
    ...
    else if (prefs.alertphonenumberlink === true && sumoreplycontent.search(/\(?\b([0-9O]{3})\)?[-. ]?([0-9O]{3})[-. ]?([0-9O]{4})\b/) != -1) {
        return "phone number";
    }
    else if (prefs.alertphonenumberlink === true && sumoreplycontent.search(/\b((https?|ftp|file):\/\/)?[a-z0-9]*[.][a-z]*\/[a-z0-9]+/) != -1 && sumoreplycontent.search(/[:./ \s]mozilla.(org|com)/) == -1) {
        return "link";
    }
